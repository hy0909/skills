...
**Form fields are parallel, not a chain.** 한 화면(폼/모달)에서 입력하는 필드들은 시간 순서가 없다 — 수량 → 수출국 → 제품처럼 순차 체인으로 그리지 말 것. 같은 col에 세로로 나란히 배치해 화면 노드에서 병렬로 분기시키고(edge label `병렬`), 모두 "버튼 활성 조건" decision(예: `필수 모두 입력?`)으로 모은다 — **필수 입력 노드 전부가 이 decision으로 엣지를 가져야 한다**(하나만 연결하면 로직 오류). 이 decision은 입력 노드들 바로 오른쪽 인접 col에 둬서 모든 수렴 엣지가 인접 연결이 되게 하고, 필드값 유효성 검증 체인(단위·개수·총량 등)은 그 decision 뒤에 배치한다. **선택 입력 노드는 이 decision으로 수렴시키지 않는다** — 검사 대상이 아닌데 검사 마름모로 선이 들어가면 검사하는 것처럼 읽힌다. 선택 입력은 화면에서 병렬로 뻗는 가지로만 두고, details에 `입력 여부 검사 없음`을 명시한다. decision details에는 `필수 항목만 검사: …`로 검사 대상을 못박는다. **필수 입력 노드는 라벨 끝에 `*`, 선택 입력은 라벨에 `(선택)`을 붙인다** — 플러그인이 타이틀 영역에서 `*`를 **`(필수항목)`**(빨간색), `(선택)`을 **`(선택항목)`**으로 변환해 명확히 표기하고, 범례에도 `(필수항목) 필수 입력 · (선택항목) 선택 입력`을 자동 표기한다. JSON 라벨은 짧게 `*`/`(선택)`으로 쓰면 된다(16자 제한은 변환 전 기준). 디자인이 실제로 입력 순서를 강제할 때만 체인을 쓴다.
...
**모든 엣지는 인접 셀만 잇는다 — 선 겹침의 유일한 예방법.** FigJam 커넥터는 경유점 제어가 불가능해서, 2칸 이상 떨어진 노드를 잇는 엣지는 중간 노드를 관통하고 라벨이 다른 라벨·노드와 겹친다. 규칙: ① 엣지는 가로/세로/대각 1칸 이내로만 ② 유효성 검증이 여러 개면 decision 하나에서 부챗살로 뿌리지 말고 **decision 체인**(마름모를 col로 나란히, 각 오류는 자기 마름모 바로 아래 row 1)으로 편다 — 코드의 순차 가드와도 일치 ③ 긴 루프백(에러→입력, 취소→목록) 엣지는 금지하고 해당 노드 details에 `수정 시 즉시 재검증`, `[취소] → 모달 닫고 목록 유지`처럼 결과를 명시하거나, [아니요]류 복귀는 인접한 `end`(exception) 노드로 종결한다 ④ 취소·닫기로 흐름이 끝나면 그 노드를 `end` 타입으로 — 뒤로 이어지는 선이 생기면 안 된다. ⑤ 같은 col 통로에서 반대 방향 대각 엣지 2개(X자 교차) 금지 — 라벨이 같은 지점에 겹친다(validator가 ERROR로 거부). 버튼 비활성·인풋 컷처럼 필드 내부 FE 동작은 흐름 노드가 아니라 note/details로 쓴다.
...
- 이 스킬의 예외: 노드 `label`(≤16자)·`details`(≤22자)는 기존 명사형·글자수 규칙을 유지한다. 이 문체 규칙은 `note` 노드 문장과 사용자에게 보내는 안내문에 적용한다.
